...
use App\Models\FinanceConfigItem;
...
class FinanceConfigController extends Controller
{
    ...
    {
        $query = FinanceConfigItem::ordered();
    ...
    {
        $items = FinanceConfigItem::byCategory($category)->active()->ordered()->get();
    ...
    {
        $request->validate([
            'category' => 'required|string|in:charge_category',
            'name'     => 'required|string|max:150',
        ]);

        $maxOrder = FinanceConfigItem::byCategory($request->input('category'))->max('sort_order') ?? -1;

        $item = FinanceConfigItem::create([
    ...
    {
        $item = FinanceConfigItem::findOrFail($id);
