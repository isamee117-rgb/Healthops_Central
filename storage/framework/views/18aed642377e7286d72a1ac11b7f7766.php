...
<div class="module-page">
    <div class="module-header">
        <div>
            <h1><i data-lucide="user-cog"></i> Doctor Master</h1>
            <p class="module-subtitle">Manage doctor profiles and information</p>
        </div>
        <div class="module-header-actions">
            <button class="btn-primary" id="btnAddDoctor" data-require-permission="doctors.create"><i data-lucide="user-plus"></i> Add Doctor</button>
        </div>
    </div>
    ...
    <div class="opd-toolbar">
        <div class="opd-search-wrap">
            ...
            <input type="text" class="opd-search-input" id="doctorSearch" placeholder="Search by name, specialization, department...">
        </div>
        <div class="opd-toolbar-right">
            <button class="opd-tool-btn opd-tool-btn--icon" id="btnDrFilter" onclick="toggleDrFilter(event)" title="Filter">
                <i data-lucide="filter"></i>
                <span class="opd-filter-badge" id="drFilterBadge" style="display:none">0</span>
            </button>
            <div class="opd-rows-wrap">
                <button class="opd-tool-btn opd-tool-btn--icon" id="drRowsBtn" onclick="toggleDrRowsMenu(event)" title="Rows per page">
                    <i data-lucide="layout-list"></i>
                </button>
                <div class="opd-rows-menu" id="drRowsMenu">
                    <div class="opd-rows-head">Rows per page</div>
                    <button onclick="setDrRowsPer(10)">10 rows</button>
                    <button onclick="setDrRowsPer(25)">25 rows</button>
                    <button onclick="setDrRowsPer(50)">50 rows</button>
                    <button onclick="setDrRowsPer(100)">100 rows</button>
                </div>
            </div>
            <div class="opd-col-vis-wrap">
                <button class="opd-tool-btn opd-tool-btn--icon" title="Column Visibility" onclick="toggleDrColVis(event)">
                    <i data-lucide="columns-3"></i>
                </button>
                <div class="opd-col-vis-menu" id="drColVisMenu">
                    <div class="opd-col-vis-head">
                        <span>Column Visibility</span>
                        <button class="opd-col-vis-selall" onclick="drColVisSelectAll()">Select All</button>
                    </div>
                    <div class="opd-col-vis-list" id="drColVisList">
                        <label><input type="checkbox" checked data-col="0"> Photo</label>
                        <label><input type="checkbox" checked data-col="1"> Doctor ID</label>
                        <label><input type="checkbox" checked data-col="2"> Name & Degrees</label>
                        <label><input type="checkbox" checked data-col="3"> Specialization</label>
                        <label><input type="checkbox" checked data-col="4"> Department</label>
                        <label><input type="checkbox" checked data-col="5"> Experience</label>
                        <label><input type="checkbox" checked data-col="6"> Employment</label>
                    ...
                    <div class="opd-col-vis-foot">
                        <button class="opd-col-vis-save" onclick="applyDrColVis()">Save</button>
                    </div>
                </div>
            </div>
            <div class="opd-export-wrap">
                <button class="opd-tool-btn" onclick="toggleDrExportMenu(event)" title="Export" style="padding:0 10px">
                    <i data-lucide="upload"></i>
                    <i data-lucide="chevron-down" style="width:13px;height:13px;margin-left:2px"></i>
                </button>
                <div class="opd-export-menu" id="drExportMenu">
                    <button onclick="exportDr('csv')"><i data-lucide="file-spreadsheet"></i> CSV</button>
                    <button onclick="exportDr('print')"><i data-lucide="printer"></i> Print</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="opd-filter-pane" id="drFilterPane" style="display:none">
        <div class="opd-filter-pane-head">
            <span style="font-size:13px;font-weight:700;color:var(--color-foreground)">Filter Doctors</span>
            <button class="opd-filter-close" onclick="toggleDrFilter(event)">
        ...
        <div class="opd-filter-pane-body">
            <div class="opd-filter-field">
                ...
                <div class="opd-cs-wrap" id="drCsStatus" data-target="drStatusFilter" data-placeholder="All Status"></div>
                <input type="hidden" id="drStatusFilter">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Department</label>
                <div class="opd-cs-wrap" id="drCsDept" data-target="drDeptFilter" data-placeholder="All Departments"></div>
                <input type="hidden" id="drDeptFilter">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Employment Type</label>
                <div class="opd-cs-wrap" id="drCsType" data-target="drTypeFilter" data-placeholder="All Employment"></div>
                <input type="hidden" id="drTypeFilter">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Joining Date From</label>
                <div class="opd-dp-wrap" id="drDpDateFrom" data-target="drDateFrom" data-placeholder="Select date"></div>
                <input type="hidden" id="drDateFrom">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Joining Date To</label>
                <div class="opd-dp-wrap" id="drDpDateTo" data-target="drDateTo" data-placeholder="Select date"></div>
                <input type="hidden" id="drDateTo">
            </div>
        </div>
        <div class="opd-filter-pane-foot">
            <button class="opd-filter-reset" onclick="resetDrFilters()">
            ...
            <button class="opd-filter-apply" onclick="applyDrFilters()">
        ...
        <div style="overflow-x:auto">
            <table class="data-table" id="doctorsTable">
                ...
                <tbody id="doctorsTableBody">
                    <tr><td colspan="10"><div class="empty-state"><i data-lucide="stethoscope"></i><p>Loading doctors...</p></div></td></tr>
                </tbody>
            </table>
        </div>
        <div class="opd-pagination" id="drPagination">
            <span class="opd-page-info" id="drTableInfo">Showing 0–0 of 0 records</span>
            <div class="opd-page-btns">
                <button class="opd-page-btn" id="drPrevPage" disabled>
                ...
                <div class="opd-page-nums" id="drPageNums"></div>
                <button class="opd-page-btn" id="drNextPage" disabled>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="doctorDetailSheet" style="width:700px">
    <div class="offcanvas-header" style="background:var(--midnight-blue);color:#fff;padding:16px 20px">
        <h5 class="offcanvas-title" id="doctorDetailTitle">Doctor Details</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas"></button>
    </div>
    <div class="offcanvas-body" id="doctorDetailBody" style="padding:0"></div>
    <div class="offcanvas-footer" id="doctorDetailFooter" style="display:flex;justify-content:space-between;padding:12px 20px;border-top:1px solid var(--color-border)"></div>
</div>

<div class="offcanvas offcanvas-end" tabindex="-1" id="doctorFormSheet" style="width:800px">
    <div class="offcanvas-header" style="background:var(--midnight-blue);color:#fff;padding:16px 20px">
        <h5 class="offcanvas-title" id="doctorFormTitle">Add New Doctor</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas"></button>
    </div>
    <div class="offcanvas-body" id="doctorFormBody" style="padding:20px"></div>
    <div class="offcanvas-footer" style="display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid var(--color-border)">
        ...
        <button class="btn-primary" id="btnSaveDoctor"><i data-lucide="check"></i> Save Doctor</button>
    </div>
</div>
...
<script src="<?php echo e(asset('js/doctors.js')); ?>?v=<?php echo e(filemtime(public_path('js/doctors.js'))); ?>"></script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\healthops\resources\views/pages/doctors.blade.php ENDPATH**/ ?>