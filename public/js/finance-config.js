$(document).ready(function () {
    ...
    /* ── Dropdown Config ── */
    function loadAll() {
        $.get('/api/finance-config').done(function (data) {
            configData = data || {};
            renderAll();
        });
    }
    ...
    function renderAll() {
        $('#finConfigContainer .card[data-category]').each(function () {
    ...
    function loadSeries() {
        $.get('/api/finance-number-series').done(function (data) {
            seriesData = data || [];
            renderSeries();
        });
    }
    ...
    function renderSeries() {
        var $container = $('#finNumberSeriesContainer');
        $container.empty();
                    ...
                    '<div class="series-body">' +
                        '<div class="row g-3">' +
                            '<div class="col-12">' +
                                ...
                                '<input type="text" class="form-control series-prefix" value="' + esc(s.prefix) + '" maxlength="20" placeholder="e.g. CHG-">' +
                                '<div class="form-text" style="font-size:11px">Characters prepended to each generated ID (e.g. <code>CHG-</code>, <code>SVC-</code>)</div>' +
                            '</div>' +
                            '<div class="col-6">' +
    ...
    $(document).on('click', '.btn-save-series', function () {
        ...
        $.ajax({
            url: '/api/finance-number-series/' + key,
            method: 'PUT',
            contentType: 'application/json',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
