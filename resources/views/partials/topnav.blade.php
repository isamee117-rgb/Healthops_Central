...
<header class="topnav">
    ...
    {{-- Left spacer — mirrors the right side to keep search centered --}}
    <div style="flex:1;min-width:0"></div>

    {{-- Center: Global Search (fixed width so it stays truly centered) --}}
    ...
    <div style="flex:1;min-width:0;text-align:right;padding-left:16px">
        <div id="topnavHospitalName" style="font-size:13px;font-weight:600;color:#fff;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $hnName }}</div>
        @if($hnShortName)
        <div id="topnavHospitalShortName" style="font-size:11px;color:rgba(255,255,255,0.5);line-height:1.2;white-space:nowrap">{{ $hnShortName }}</div>
        @else
