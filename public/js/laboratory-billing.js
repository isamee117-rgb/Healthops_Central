$(document).ready(function() {
    ...
    function loadRevenue(period) {
        $.get('/api/laboratory-billing/revenue', { period: period }, function(d) {
            ...
            if (!d.paymentCategories || d.paymentCategories.length === 0) {
    ...
    function openTxnDetail(txnId) {
        ...
        $.get('/api/laboratory-billing/transactions/' + txnId, function(txn) {
            ...
            if (txn.items && txn.items.length > 0) {
                ...
                itemsHtml = '<p style="font-size:13px;color:var(--color-muted-foreground)">No test items recorded.</p>';
            }
    ...
    function openIpdOrderDetail(orderId) {
        ...
        $.get('/api/laboratory-billing/ipd-order/' + orderId, function(txn) {
            $('#txnDetailId').text('Order ID: ' + txn.orderId);
            ...
            if (txn.items && txn.items.length > 0) {
                ...
                itemsHtml = '<p style="font-size:13px;color:var(--color-muted-foreground)">No tests found.</p>';
            }
    ...
    function loadPendingOPD() {
        $.get('/api/laboratory-billing/pending-opd', function(d) {
            ...
            if (!d.items || d.items.length === 0) {
                $tb.append('<tr><td colspan="6" style="padding:20px;text-align:center;color:var(--color-muted-foreground);font-size:13px">No pending OPD payments</td></tr>');
                return;
            }
            d.items.forEach(function(item) {
                $tb.append(
                    '<tr style="border-bottom:1px solid var(--color-border)">' +
                        ...
                        '<td style="padding:8px 14px;text-align:right;font-size:13px;font-weight:600;color:#f97316;font-family:monospace">' + fmt(item.amountDue) + '</td>' +
                        '<td style="padding:8px 14px;text-align:center;font-size:12px;color:var(--color-muted-foreground)">' + esc(item.daysPending) + '</td>' +
                        '<td style="padding:8px 14px;text-align:center"><button class="collect-opd-btn" data-txn-id="' + esc(item.transactionId) + '" style="padding:5px 12px;background:var(--aquamint);border:none;border-radius:6px;font-size:11px;font-weight:600;color:var(--midnight-blue);cursor:pointer">Collect</button></td>' +
                    '</tr>'
                );
            });
        });
    ...
    $(document).on('click', '.collect-opd-btn', function() {
        pendingCollectTxnId = $(this).data('txn-id');
        $('#collectPayModal').css('display', 'flex');
    });

    $('#collectPayConfirm').on('click', function() {
        if (!pendingCollectTxnId) return;
        var mode = $('#collectPayMode').val();
        $.ajax({
    ...
    function loadPendingIPD() {
        $.get('/api/laboratory-billing/pending-ipd', function(d) {
            ...
            if (!d.items || d.items.length === 0) {
                $tb.append('<tr><td colspan="5" style="padding:20px;text-align:center;color:var(--color-muted-foreground);font-size:13px">No pending IPD lab charges</td></tr>');
                return;
            }
            d.items.forEach(function(item) {
                $tb.append(
                    '<tr style="border-bottom:1px solid var(--color-border)">' +
                        ...
                        '<td style="padding:8px 14px;text-align:right;font-size:13px;font-weight:600;color:#eab308;font-family:monospace">' + fmt(item.totalCharges) + '</td>' +
                        '<td style="padding:8px 14px;font-size:12px;color:var(--color-muted-foreground)">' + esc(item.lastUpdated) + '</td>' +
                        '<td style="padding:8px 14px;text-align:center"><button class="view-ipd-btn" data-order-id="' + esc(item.transactionId) + '" style="padding:5px 12px;background:var(--color-background);border:1px solid var(--color-border);border-radius:6px;font-size:11px;font-weight:600;cursor:pointer">View</button></td>' +
                    '</tr>'
                );
            });
        });
    ...
    $(document).on('click', '.view-ipd-btn', function() {
        var orderId = $(this).data('order-id');
        if (orderId && orderId.toString().startsWith('LTXN-')) {
            openTxnDetail(orderId);
        } else {
            openIpdOrderDetail(orderId);
        }
    });
