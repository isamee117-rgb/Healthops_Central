...
use App\Models\OtIntraopFormSection;
...
class OtIntraopFormSectionController extends Controller
{
    ...
    {
        $sections = OtIntraopFormSection::orderBy('sort_order')->orderBy('id')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $maxOrder = OtIntraopFormSection::max('sort_order') ?? 0;

            $section = OtIntraopFormSection::create([
                'key'        => $key,
    ...
    {
        try {
            $section    = OtIntraopFormSection::findOrFail($id);
            $updateData = [];
