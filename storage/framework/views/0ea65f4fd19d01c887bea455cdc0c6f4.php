...
<div class="module-page">
    <div class="module-header">
        <div>
            <h1><i data-lucide="receipt"></i> Doctor Fee Management</h1>
            <p class="module-subtitle">Manage doctor fees across all service categories</p>
        </div>
        <div class="module-header-actions">
            <button class="btn-primary" id="btnAddDoctorFee"><i data-lucide="plus-circle"></i> Add Doctor Fee</button>
        </div>
    </div>
    ...
    <div class="opd-toolbar">
        <div class="opd-search-wrap">
            ...
            <input type="text" class="opd-search-input" id="feeSearch" placeholder="Search by doctor name, category, procedure...">
        </div>
        <div class="opd-toolbar-right">
            <button class="opd-tool-btn opd-tool-btn--icon" id="btnDfFilter" onclick="toggleDfFilter(event)" title="Filter">
                <i data-lucide="filter"></i>
                <span class="opd-filter-badge" id="dfFilterBadge" style="display:none">0</span>
            </button>
            <div class="opd-rows-wrap">
                <button class="opd-tool-btn opd-tool-btn--icon" id="dfRowsBtn" onclick="toggleDfRowsMenu(event)" title="Rows per page">
                    <i data-lucide="layout-list"></i>
                </button>
                <div class="opd-rows-menu" id="dfRowsMenu">
                    <div class="opd-rows-head">Rows per page</div>
                    <button onclick="setDfRowsPer(10)">10 rows</button>
                    <button onclick="setDfRowsPer(25)">25 rows</button>
                    <button onclick="setDfRowsPer(50)">50 rows</button>
                    <button onclick="setDfRowsPer(100)">100 rows</button>
                </div>
            </div>
            <div class="opd-col-vis-wrap">
                <button class="opd-tool-btn opd-tool-btn--icon" title="Column Visibility" onclick="toggleDfColVis(event)">
                    <i data-lucide="columns-3"></i>
                </button>
                <div class="opd-col-vis-menu" id="dfColVisMenu">
                    <div class="opd-col-vis-head">
                        <span>Column Visibility</span>
                        <button class="opd-col-vis-selall" onclick="dfColVisSelectAll()">Select All</button>
                    </div>
                    <div class="opd-col-vis-list" id="dfColVisList">
                        <label><input type="checkbox" checked data-col="0"> Fee ID</label>
                        <label><input type="checkbox" checked data-col="1"> Doctor</label>
                        <label><input type="checkbox" checked data-col="2"> Category</label>
                        <label><input type="checkbox" checked data-col="3"> Visit Type / Procedure</label>
                        <label><input type="checkbox" checked data-col="4"> Fee Amount</label>
                        <label><input type="checkbox" checked data-col="5"> Date Added</label>
                    </div>
                    <div class="opd-col-vis-foot">
                        <button class="opd-col-vis-save" onclick="applyDfColVis()">Save</button>
                    </div>
                </div>
            </div>
            <div class="opd-export-wrap">
                <button class="opd-tool-btn" onclick="toggleDfExportMenu(event)" title="Export" style="padding:0 10px">
                    <i data-lucide="upload"></i>
                    <i data-lucide="chevron-down" style="width:13px;height:13px;margin-left:2px"></i>
                </button>
                <div class="opd-export-menu" id="dfExportMenu">
                    <button onclick="exportDf('csv')"><i data-lucide="file-spreadsheet"></i> CSV</button>
                    <button onclick="exportDf('print')"><i data-lucide="printer"></i> Print</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="opd-filter-pane" id="dfFilterPane" style="display:none">
        <div class="opd-filter-pane-head">
            <span style="font-size:13px;font-weight:700;color:var(--color-foreground)">Filter Fees</span>
            <button class="opd-filter-close" onclick="toggleDfFilter(event)">
        ...
        <div class="opd-filter-pane-body" style="grid-template-columns:repeat(4,1fr)">
            <div class="opd-filter-field">
                ...
                <div class="opd-cs-wrap" id="dfCsCategory" data-target="dfCatFilter" data-placeholder="All Categories"></div>
                <input type="hidden" id="dfCatFilter">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Doctor</label>
                <div class="opd-cs-wrap" id="dfCsDoctor" data-target="dfDocFilter" data-placeholder="All Doctors"></div>
                <input type="hidden" id="dfDocFilter">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Date From</label>
                <div class="opd-dp-wrap" id="dfDpDateFrom" data-target="dfDateFrom" data-placeholder="Select date"></div>
                <input type="hidden" id="dfDateFrom">
            </div>
            <div class="opd-filter-field">
                <label class="opd-filter-label">Date To</label>
                <div class="opd-dp-wrap" id="dfDpDateTo" data-target="dfDateTo" data-placeholder="Select date"></div>
                <input type="hidden" id="dfDateTo">
            </div>
        </div>
        <div class="opd-filter-pane-foot">
            <button class="opd-filter-reset" onclick="resetDfFilters()">
            ...
            <button class="opd-filter-apply" onclick="applyDfFilters()">
        ...
        <div style="overflow-x:auto">
            <table class="data-table" id="feesTable">
                ...
                <tbody id="feesTableBody">
                    <tr><td colspan="7"><div class="empty-state"><i data-lucide="receipt"></i><p>Loading fees...</p></div></td></tr>
                </tbody>
            </table>
        </div>
        <div class="opd-pagination" id="dfPagination">
            <span class="opd-page-info" id="dfTableInfo">Showing 0–0 of 0 records</span>
            <div class="opd-page-btns">
                <button class="opd-page-btn" id="dfPrevPage" disabled>
                ...
                <div class="opd-page-nums" id="dfPageNums"></div>
                <button class="opd-page-btn" id="dfNextPage" disabled>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="feeFormSheet" style="width:800px">
    <div class="offcanvas-header" style="background:var(--midnight-blue);color:#fff;padding:16px 20px">
        <h5 class="offcanvas-title" id="feeFormTitle">Add Doctor Fee</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas"></button>
    </div>
    <div class="offcanvas-body" id="feeFormBody" style="padding:20px"></div>
    <div class="offcanvas-footer" style="display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid var(--color-border)">
        <button class="btn-outline" id="btnClearFeeForm"><i data-lucide="x"></i> Clear Form</button>
        <button class="btn-primary" id="btnSaveFees" disabled><i data-lucide="check"></i> Add Fees</button>
    </div>
</div>

<div class="offcanvas offcanvas-end" tabindex="-1" id="feeEditSheet" style="width:500px">
    <div class="offcanvas-header" style="background:var(--midnight-blue);color:#fff;padding:16px 20px">
        <h5 class="offcanvas-title" id="feeEditTitle">Edit Doctor Fee</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas"></button>
    </div>
    <div class="offcanvas-body" id="feeEditBody" style="padding:20px"></div>
    <div class="offcanvas-footer" style="display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid var(--color-border)">
        <button class="btn-outline" data-bs-dismiss="offcanvas">Cancel</button>
        <button class="btn-primary" id="btnUpdateFee"><i data-lucide="check"></i> Update Fee</button>
    </div>
</div>
...
<script src="<?php echo e(asset('js/doctor-fees.js')); ?>?v=<?php echo e(filemtime(public_path('js/doctor-fees.js'))); ?>"></script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\healthops\resources\views/pages/doctor-fees.blade.php ENDPATH**/ ?>